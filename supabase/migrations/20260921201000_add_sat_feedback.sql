...
drop policy if exists "Anyone can read public SAT feedback" on public.sat_feedback;
create policy "Anyone can read public SAT feedback"
on public.sat_feedback
for select
to anon, authenticated
using (is_public = true);

revoke select on public.sat_feedback from anon, authenticated;
grant select (id, vibe, message, created_at) on public.sat_feedback to anon, authenticated;
