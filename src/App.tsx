...
export default function App(){
  ...
  if(view==='practice'&&current&&qpdf)return <main className="practice">
    <header className="practice-topbar">
      ...
      <div className="question-nav"><button onClick={()=>goTo(i-1)} disabled={i===0}><ChevronLeft/> Previous</button><span>{moduleLabel(current.module)} · Q{current.number}</span><button className="next-top" onClick={next} disabled={!currentRec}>{i+1===qs.length?'Finish':'Next'} <ChevronRight/></button></div>
    </header>
    <div className="practice-grid">
      ...
      <aside className="answer"><h2>Your answer</h2>{current.responseType==='multiple-choice'?<div className="choices">{['A','B','C','D'].map(c=><button disabled={!!currentRec} className={selected===c?'selected':''} onClick={()=>setSelected(c)} key={c}>{c}</button>)}</div>:<input disabled={!!currentRec||submitted} value={selected} onChange={e=>setSelected(e.target.value)} placeholder="Type your answer"/>}{!currentRec&&!submitted&&<button className="primary" disabled={!selected.trim()} onClick={submit}>Submit answer</button>}{submitted&&current.responseType==='student-produced'&&!currentRec&&<div className="feedback">Official answer: <b>{current.correctAnswer}</b><div><button onClick={()=>record(true,true)}>I got it right</button><button onClick={()=>record(false,true)}>I got it wrong</button></div></div>}{currentRec&&<div className={currentRec.correct?'feedback good':'feedback bad'}><b>{currentRec.correct?'Correct':'Not quite'}</b><span>Correct answer: {current.correctAnswer}</span><span>Time: {fmt(currentRec.elapsedMs)}</span></div>}<p className="review-note">Explanations are available after you finish the session.</p></aside>
    </div>
  </main>
