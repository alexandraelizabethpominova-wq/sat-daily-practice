...
export default function App(){
  ...
  if(view==='practice'&&current&&qpdf)return <main className="practice">
    <header className="practice-topbar"><button className="logo" onClick={()=>setView('home')}><Sparkles/> SAT Daily</button><div className="progress-block"><b>Question {i+1} of {qs.length}</b><progress value={i+1} max={qs.length}/></div><div className="question-nav"><AlexButton tone="secondary" startIcon={<ChevronLeft size={18}/>} onClick={()=>goTo(i-1)} disabled={i===0}>Previous</AlexButton><span>{moduleLabel(current.module)} · Q{current.number}</span><AlexButton endIcon={<ChevronRight size={18}/>} onClick={next} disabled={!currentRec}>{i+1===qs.length?'Finish':'Next'}</AlexButton></div></header>
    <div className="practice-grid"><section className="question-card"><div className="focus"><b>Question {current.number}</b></div><SourceSlice pdfKey="questions" bytes={qpdf} page={current.sourcePage} questionNumber={current.number} alt={`${moduleLabel(current.module)} question ${current.number}`}/></section><aside className="answer"><h2>Your answer</h2>{current.responseType==='multiple-choice'?<div className="choices">{['A','B','C','D'].map(c=><AlexChoiceButton disabled={!!currentRec} selected={selected===c} onClick={()=>setSelected(c)} key={c} label={c}/>)}</div>:<AlexTextField disabled={!!currentRec||submitted} value={selected} onChange={e=>setSelected(e.target.value)} placeholder="Type your answer"/>}{!currentRec&&!submitted&&<AlexButton fullWidth disabled={!selected.trim()} onClick={submit} sx={{mt:1.5}}>Submit answer</AlexButton>}{submitted&&current.responseType==='student-produced'&&!currentRec&&<div className="feedback">Official answer: <b>{current.correctAnswer}</b><div><AlexButton tone="secondary" onClick={()=>record(true,true)}>I got it right</AlexButton><AlexButton tone="secondary" onClick={()=>record(false,true)}>I got it wrong</AlexButton></div></div>}{currentRec&&<div className={currentRec.correct?'feedback good':'feedback bad'}><b>{currentRec.correct?'Correct':'Not quite'}</b><span>Correct answer: {current.correctAnswer}</span><span>Time: {fmt(currentRec.elapsedMs)}</span></div>}<p className="review-note">Explanations are available after you finish the session.</p></aside></div>
  </main>
