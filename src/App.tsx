...
import PracticeOptionCard from './design-system/molecules/PracticeOptionCard'
import SideNavigation from './design-system/molecules/SideNavigation'
...
export default function App(){
  ...
  function beginPractice(nextMode=settings.mode){if(!qpdf){setView('sources');return}const nextSettings={...settings,mode:nextMode};setSettings(nextSettings);const set=choose(nextSettings,attempts);setQs(set);setSid(uid());setStarted(new Date().toISOString());setCurrentAttempts([]);setI(0);setSelected('');setSubmitted(false);setQStart(Date.now());setView('practice')}
  ...
  function resetHistory(){if(!window.confirm('Delete all practice history and statistics? Your settings and uploaded question sources will be kept.'))return;clearHistory();setAttempts([]);setSessions([]);setCurrentAttempts([]);setQs([]);setSid('');setStarted('');setI(0);setSelected('');setSubmitted(false);setView('home')}

  if(view==='practice'&&current&&qpdf)return <main className="practice">
    <header className="practice-topbar"><button className="logo practice-logo" onClick={()=>setView('home')}><Sparkles/> SAT Daily</button><div className="progress-block"><div><b>{moduleLabel(current.module)}</b><span>Question {i+1} of {qs.length}</span></div><progress value={i+1} max={qs.length}/></div><div className="question-nav"><AlexButton tone="secondary" startIcon={<ChevronLeft size={18}/>} onClick={()=>goTo(i-1)} disabled={i===0}>Previous</AlexButton><AlexButton endIcon={<ChevronRight size={18}/>} onClick={next} disabled={!currentRec}>{i+1===qs.length?'Finish':'Next'}</AlexButton></div></header>
    <div className="practice-workspace"><section className="question-panel"><div className="question-heading"><div><span>QUESTION {current.number}</span><b>{current.subject==='math'?'Math':'Reading & Writing'}</b></div><AlexStatusChip>READY</AlexStatusChip></div><SourceSlice pdfKey="questions" bytes={qpdf} page={current.sourcePage} questionNumber={current.number} alt={`${moduleLabel(current.module)} question ${current.number}`}/></section><aside className="answer-panel"><div><p className="answer-kicker">Your answer</p><h2>Choose the best answer.</h2><p className="answer-helper">Answer this question, then use Next to continue.</p></div>{current.responseType==='multiple-choice'?<div className="choices">{['A','B','C','D'].map(c=><AlexChoiceButton disabled={!!currentRec} selected={selected===c} onClick={()=>setSelected(c)} key={c} label={c}/>)}</div>:<AlexTextField disabled={!!currentRec||submitted} value={selected} onChange={e=>setSelected(e.target.value)} placeholder="Type your answer"/>}{!currentRec&&!submitted&&<AlexButton fullWidth disabled={!selected.trim()} onClick={submit}>Submit answer</AlexButton>}{submitted&&current.responseType==='student-produced'&&!currentRec&&<div className="feedback">Official answer: <b>{current.correctAnswer}</b><div><AlexButton tone="secondary" onClick={()=>record(true,true)}>I got it right</AlexButton><AlexButton tone="secondary" onClick={()=>record(false,true)}>I got it wrong</AlexButton></div></div>}{currentRec&&<div className={currentRec.correct?'feedback good':'feedback bad'}><b>{currentRec.correct?'Correct':'Not quite'}</b><span>Correct answer: {current.correctAnswer}</span><span>Time: {fmt(currentRec.elapsedMs)}</span></div>}<p className="review-note">Explanation will be available in your session review.</p></aside></div>
  </main>
  ...
  if(view==='stats')return <main className="shell"><Nav current="stats" setView={setView}/><div className="page-heading"><div><p className="eyebrow">History</p><h1>Your practice trends</h1></div>{attempts.length>0&&<AlexButton tone="secondary" onClick={resetHistory}>Clear history & start fresh</AlexButton>}</div><div className="stats"><Stat icon={<Target/>} label="Accuracy" value={attempts.length?`${accuracy}%`:'—'}/><Stat icon={<Clock3/>} label="Avg. time" value={attempts.length?fmt(avg):'—'}/><Stat icon={<BookOpen/>} label="Sessions" value={String(sessions.length)}/><Stat icon={<BarChart3/>} label="Questions seen" value={`${new Set(attempts.map(a=>a.questionId)).size}/${QUESTION_BANK.length}`}/></div>{attempts.length===0&&<section className="card empty-history"><h2>Fresh start</h2><p>No practice history is stored yet. Your next session will begin building new statistics.</p></section>}</main>

  if(view==='settings')return <main className="shell"><Nav current="settings" setView={setView}/><h1>Practice setup</h1><section className="card settings settings-grid"><div className="settings-field"><AlexDropdown id="practice-subject" label="Subject" value={settings.mode} options={[{value:'both',label:'English + Math'},{value:'english',label:'English only'},{value:'math',label:'Math only'}]} onChange={mode=>setSettings({...settings,mode})}/></div><div className="settings-field"><AlexNumberField label="Questions per session" value={settings.questionsPerSession} min={3} max={30} onChange={questionsPerSession=>setSettings({...settings,questionsPerSession})}/></div><div className="settings-actions"><AlexButton tone="secondary" onClick={resetHistory}>Clear history & start fresh</AlexButton></div><p className="muted">This removes saved attempts, sessions, accuracy, timing history, and question-selection history. Your settings and uploaded PDF sources are kept.</p></section></main>
  ...
  return <main className="app-shell"><SideNavigation primary={[{label:'Study Plan',active:view==='home',onClick:()=>setView('home')},{label:'Practice Tests',active:view==='practice',onClick:start},{label:'Question Bank',onClick:()=>setView('settings')},{label:'Performance',active:view==='stats',onClick:()=>setView('stats')}]} secondary={[{label:'Resources',active:view==='sources',onClick:()=>setView('sources')}]} /><section className="dashboard"><div className="dashboard-header"><p className="eyebrow">Practice Tests</p><h1>SAT Practice Tests</h1><p>Build a focused practice session, review your performance, and keep working on the questions that need the most attention.</p></div><div className="dashboard-divider"/><section className="practice-options"><div className="section-title"><h2>Practice Test Options</h2><p>Choose a full mixed session or focus on one SAT section.</p></div><div className="option-grid"><PracticeOptionCard title="Full practice session" description="Reading & Writing + Math" actions={[{status:'READY',title:'Start mixed SAT practice',detail:`${settings.questionsPerSession} questions using your adaptive question pool`,action:<AlexButton onClick={()=>beginPractice('both')}>Start</AlexButton>}]} /><PracticeOptionCard title="Single section practice" description="Focus on one section at a time" actions={[{status:'READY',title:'Reading & Writing practice',compact:true,action:<AlexButton tone="secondary" onClick={()=>beginPractice('english')}>Start</AlexButton>},{status:'READY',title:'Math practice',compact:true,action:<AlexButton tone="secondary" onClick={()=>beginPractice('math')}>Start</AlexButton>}]} /></div></section></section></main>
}
