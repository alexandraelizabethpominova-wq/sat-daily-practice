...
export default function PracticeAnswerPanel({question,selected,submitted,attempt,explanationBytes,onSelect,onSubmit}:Props){
  return <aside className="answer-panel">
    ...
    {question.responseType==='multiple-choice'?<div className="choices">{['A','B','C','D'].map(choice=><AlexChoiceButton disabled={Boolean(attempt)} selected={selected===choice} onClick={()=>onSelect(choice)} key={choice} label={choice}/>)}</div>:<AlexTextField disabled={Boolean(attempt)||submitted} value={selected} onChange={event=>onSelect(event.target.value)} placeholder="Type a number, decimal, or fraction"/>}
