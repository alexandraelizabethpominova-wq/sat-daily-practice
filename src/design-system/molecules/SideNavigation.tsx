...
function NavItem({item,collapsed}:{item:Item;collapsed:boolean}){
  const button=(
    ...
    >
      ...
      {!collapsed&&<Typography component="span" sx={{fontSize:14,fontWeight:650}}>{item.label}</Typography>}
    </ButtonBase>
  )
    ...
    >
      <Box sx={{display:'flex',alignItems:'center',gap:0.5,mb:1.2}}>
        ...
        >
          ...
          {!collapsed&&<Typography component="span" sx={{fontWeight:850}}>{brand}</Typography>}
        </ButtonBase>
        ...
        {onToggleCollapsed&&(
          <Tooltip title={collapsed?'Expand menu':'Collapse menu'} placement="right">
            <IconButton
              ...
              sx={{color:'#DCE7F7',width:30,height:30}}
            >
      ...
      {secondary.length>0&&(
        <Box sx={{mt:2.35}}>
          {!collapsed&&(
            <Box sx={{display:'flex',alignItems:'center',gap:1.1,px:1.25,mb:1}}>
              <Typography sx={{fontSize:10.5,letterSpacing:'.03em',whiteSpace:'nowrap',color:'#DCE7F7'}}>ADDITIONAL TOOLS</Typography>
              <Box sx={{height:'1px',bgcolor:'#AFC2DD',flex:1}}/>
            </Box>
          )}
          <Stack spacing={0.45}>
            {secondary.map(item=><NavItem item={item} collapsed={collapsed} key={item.key}/>) }
          </Stack>
        </Box>
      )}

      <Box sx={{mt:'auto',pt:2}}>
        <Tooltip title={collapsed?footer:''} placement="right">
          <ButtonBase
          ...
          >
            <Box sx={{width:32,height:32,borderRadius:'50%',bgcolor:'#BDF3B0',display:'grid',placeItems:'center',color:'#08275B',fontWeight:850,flex:'0 0 auto'}}>
              {footer.slice(0,1).toUpperCase()}
            </Box>
            {!collapsed&&(
              <>
                <Typography sx={{fontSize:13.5,fontWeight:700}}>{footer}</Typography>
                <Settings size={17} style={{marginLeft:'auto'}}/>
              </>
            )}
          </ButtonBase>
        </Tooltip>
